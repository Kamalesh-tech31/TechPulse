...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    label: 'Trade',
    items: [
      { id: 'stock-analysis',  name: 'Stock Analysis', Icon: LineChart, matchIds: ['stock-detail'] },
...
export const Sidebar: React.FC = () => {
  ...
  /* ── First-load stagger flag (once per session) ── */
  ...
  /* ── One-time stagger ── */
  useEffect(() => {
    if (!sessionStorage.getItem('trado_nav_staggered')) {
      setShouldStagger(true);
      sessionStorage.setItem('trado_nav_staggered', '1');
    }
  }, []);
  ...
  const LogoMark = ({ compact = false }: { compact?: boolean }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: compact ? 0 : 10 }}>
      <div style={{
        ...
        boxShadow:      '0 0 20px rgba(79,107,255,0.38), 0 2px 8px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.2)',
        flexShrink:     0,
    ...
    <div style={{
      ...
      {user.googlePicture ? (
        <img
          ...
          style={{ height: 36, width: 36, borderRadius: 10, objectFit: 'cover',
      ...
      {!compact && (
        <div style={{ minWidth: 0, flex: 1, overflow: 'hidden' }}>
          <div style={{
            color: token.text, fontWeight: 600, fontSize: 13,
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          }}>
            {user.name}
          </div>
          <div style={{
            color: token.muted, fontSize: 11, marginTop: 2,
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          }}>
            {user.email}
          </div>
      ...
      <div style={{
        ...
        {compact ? (
          ...
          <div style={{
            ...
            {walletTarget === 0
              ? <span className="skeleton-shimmer" style={{ display: 'inline-block', width: 80, height: 20 }} />
              : `₹${display}`
            }
          </div>
        )}
    ...
    return (
      ...
      >
        ...
        {opts.isDesktop && !isCollapsed && indicator && (
          <div
            ...
            style={{
              ...
              boxShadow:     '0 0 20px rgba(79, 107, 255, 0.12), inset 0 0 0 1px rgba(79,107,255,0.14)',
        ...
        {NAV_SECTIONS.map((section, sectionIdx) => (
          <div key={`section-${sectionIdx}`} style={{ marginBottom: isCollapsed ? 4 : 8 }}>
            ...
            {isCollapsed && sectionIdx > 0 && (
              <div style={{ height: 1, background: 'rgba(255,255,255,0.055)', margin: '6px 8px' }} />
            )}
            ...
            {section.items.map(item => {
              ...
              const { Icon } = item;
              const showStaticActive = active && (isCollapsed || !opts.isDesktop);

              const staggerStyle: React.CSSProperties = shouldStagger
                ? { opacity: 0, animation: `navItemIn 280ms ${idx * 35}ms ease-out forwards` }
                : {};

              return (
                <div
                  key={item.id}
                  style={{ position: 'relative', ...staggerStyle }}
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background:     showStaticActive ? 'rgba(79,107,255,0.12)' : 'transparent',
                      ...
                      transition:     'background 200ms ease-out, box-shadow 200ms ease-out, color 200ms ease-out',
                      minHeight:      40,
                    }}
                  >
                    <span
                    ...
                    {!isCollapsed && (
                      <span
                        ...
                        style={{
                          ...
                          overflow:   'hidden',
                          transition: 'max-width 250ms ease-out, opacity 200ms ease-out, color 200ms ease-out',
                        }}
                      >
                        {item.name}
                      </span>
  ...
  const LogoutButton = ({ compact = false }: { compact?: boolean }) => (
    <button
      ...
      onMouseEnter={e => {
        ...
        (e.currentTarget as HTMLButtonElement).style.background = 'rgba(240,87,107,0.07)';
      }}
      onMouseLeave={e => {
        (e.currentTarget as HTMLButtonElement).style.color      = 'rgba(240,87,107,0.7)';
  ...
  const DesktopSidebar = () => (
    <aside
      style={{
        ...
        height:         '100vh',
        flexShrink:     0,
        transition:     'width 280ms cubic-bezier(0.4,0,0.2,1), padding 280ms ease-out',
        overflowX:      'hidden',
        overflowY:      'auto',
        zIndex:         30,
        userSelect:     'none',
    ...
    >
      ...
      <div style={{ marginTop: 'auto' }}>
        ...
        <div style={{ height: 1, background: 'rgba(255,255,255,0.055)', margin: '12px 4px' }} />
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <button
          ...
          onMouseLeave={e => {
            (e.currentTarget as HTMLButtonElement).style.background = 'transparent';
            (e.currentTarget as HTMLButtonElement).style.color = token.secondary;
          }}
        >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <button
            ...
            style={{
              ...
              background: 'transparent', cursor: 'pointer', color: token.secondary,
              transition: 'background 200ms ease-out, color 200ms ease-out',
            }}
            onMouseEnter={e => {
              (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.06)';
  ...
  return (
    <>
      ...
      */}
      <div className="md:hidden" style={{ width: 0, flexShrink: 0, overflow: 'visible' }}>
        <MobileBar />
        <MobileDrawer />
      </div>
    </>
