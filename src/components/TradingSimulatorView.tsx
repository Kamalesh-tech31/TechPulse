...
export const TradingSimulatorView: React.FC = () => {
  ...
  const totalReturnPercentage = totalHoldingsCost > 0 
    ? (totalProfitLoss / totalHoldingsCost) * 100 
    : 0;
  ...
  const growthData = [
    ...
    { name: 'Day 4', Value: baseline + Math.round(netGain * 0.20 - 800) },
    { name: 'Day 5', Value: baseline + Math.round(netGain * 0.55 + 2400) },
    { name: 'Day 6', Value: baseline + Math.round(netGain * 0.80 - 1500) },
  ...
  const sectorAllocation = Object.entries(sectorDataMap).map(([sector, value]) => ({
    name: sector,
    value: Math.round(value)
  }));

  // Slate & Signal colors for sector charts
  const COLORS = ['#4F6BFF', '#6B82FF', '#8B9BFF', '#8B93A7', '#64708D', '#46506A'];
  ...
  return (
    <div className="space-y-6 p-6 lg:p-8 max-w-7xl mx-auto text-gray-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/[0.04] pb-5">
        <div>
          <h1 className="text-3xl font-display font-bold text-white tracking-tight">Holdings</h1>
          <p className="text-gray-400 text-sm mt-1">Track asset performance, allocation weightings, and real-time equity growth indices.</p>
        </div>
        <button
          onClick={() => {
          ...
          style={{ background: 'var(--color-trado-accent)', boxShadow: '0 0 15px rgba(79, 107, 255, 0.3)' }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glassmorphism p-5 rounded-2xl border-white/[0.04] relative overflow-hidden">
          ...
          <span className="text-[10px] font-mono text-gray-500 uppercase tracking-wider block">Total Portfolio Value</span>
          <h2 className="text-2xl font-mono font-bold text-white mt-1.5">
            ₹{totalPortfolioValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </h2>
          <div className="flex items-center gap-1.5 mt-2 text-xs">
            <span className={`font-mono font-medium flex items-center ${totalProfitLoss >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
              {totalProfitLoss >= 0 ? '+' : ''}{totalReturnPercentage.toFixed(2)}% net returns
            </span>
          </div>
        </div>
        ...
        <div className="glassmorphism p-5 rounded-2xl border-white/[0.04] relative overflow-hidden">
          ...
          <span className="text-[10px] font-mono text-gray-500 uppercase tracking-wider block">Available Virtual Cash</span>
          ...
          <span className="text-[10px] text-gray-500 font-mono mt-2 block uppercase">Liquid Capital Reserves</span>
        </div>
        ...
        <div className="glassmorphism p-5 rounded-2xl border-white/[0.04] relative overflow-hidden">
          ...
          <span className="text-[10px] font-mono text-gray-500 uppercase tracking-wider block">Total Profit/Loss</span>
          <h2 className={`text-2xl font-mono font-bold mt-1.5 ${totalProfitLoss >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
            {totalProfitLoss >= 0 ? '+' : ''}₹{totalProfitLoss.toLocaleString(undefined, { maximumFractionDigits: 2 })}
          </h2>
          <div className="flex items-center gap-1 mt-2 text-xs font-mono">
            {totalProfitLoss >= 0 ? (
              <span className="text-emerald-400 flex items-center gap-0.5"><ArrowUpRight className="h-3 w-3" /> Profit</span>
            ) : (
              <span className="text-rose-400 flex items-center gap-0.5"><ArrowDownRight className="h-3 w-3" /> Loss</span>
            )}
          </div>
        </div>
        ...
        <div className="glassmorphism p-5 rounded-2xl border-white/[0.04] relative overflow-hidden">
          ...
          <span className="text-[10px] font-mono text-gray-500 uppercase tracking-wider block">Total holdings</span>
          <h2 className="text-2xl font-mono font-bold text-white mt-1.5">
            {totalHoldingsCount} <span className="text-sm font-normal text-gray-500">Assets</span>
          </h2>
          <span className="text-[10px] text-gray-500 font-mono mt-2 block uppercase">Unique Equity Positions</span>
        </div>
      ...
      <div className="glassmorphism rounded-2xl p-6 border-white/[0.04]">
        <h2 className="font-display font-bold text-lg text-white mb-4 flex items-center gap-2">
          <Briefcase className="h-5 w-5" style={{ color: 'var(--color-trado-accent)' }} />
        ...
        {holdings.length === 0 ? (
          <div className="text-center py-16 text-gray-500 text-sm border border-dashed border-white/[0.05] rounded-xl bg-white/[0.005]">
            You have no active holdings. Select "Browse Stock Market" to explore and acquire virtual shares of Nifty 50 companies.
          </div>
        ) : (
          <>
            {/* Desktop Table View */}
                  ...
                  <div key={h.symbol} className="py-4 space-y-3 cursor-pointer" onClick={() => toggleExpand(h.symbol)}>
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <span className="font-sans font-semibold text-white text-base block">{h.symbol}</span>
                          <span className="text-[10px] text-gray-500 font-sans block mt-0.5">{h.name}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-xs text-gray-400 block">Live Asset Value</span>
                    ...
                    <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs font-mono pt-2 border-t border-white/[0.02]">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-300">₹{h.totalCost.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-white/[0.02] text-xs font-mono">
                      ...
                      <div className="flex flex-col text-right">
                        <span className="text-[10px] text-gray-500 uppercase">Return (ROI)</span>
                        <span className={`font-bold flex items-center justify-end gap-0.5 ${isProfit ? 'text-emerald-400' : 'text-rose-400'}`}>
                          {isProfit ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glassmorphism rounded-2xl p-6 border-white/[0.04]">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-display font-bold text-base text-white">Portfolio Growth Index</h3>
            <span className="text-[10px] font-mono text-gray-500 uppercase">Simulated Daily Value • 7D</span>
          </div>
        ...
        <div className="glassmorphism rounded-2xl p-6 border-white/[0.04] flex flex-col justify-between">
          <div className="flex justify-between items-center mb-2">
            <h3 className="font-display font-bold text-base text-white">Sector Allocation</h3>
            <span className="text-[10px] font-mono text-gray-500 uppercase">Weightings</span>
          </div>
            ...
            <>
              <div className="h-[210px] w-full flex items-center justify-center relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none mt-[-5px]">
                  <span className="text-[8px] font-mono text-gray-500 uppercase">Allocated</span>
              ...
              <div className="space-y-1.5 max-h-[100px] overflow-y-auto pr-1">
                {sectorAllocation.map((sector, i) => {
                  const percent = ((sector.value / totalHoldingsValue) * 100).toFixed(0);
                  return (
                    <div key={sector.name} className="flex items-center justify-between text-xs font-mono">
                      <div className="flex items-center gap-1.5">
                        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }} />
                        <span className="text-gray-400 truncate max-w-[110px]">{sector.name}</span>
                      </div>
